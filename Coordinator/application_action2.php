...
function getDecisionText($decisionCode) {
    ...
    return isset($decisionMapping[$decisionCode]) ? $decisionMapping[$decisionCode] : 'Pending';
}
...
function displayData($conn) {
    ...
    $output = "";
    $sql = "SELECT * FROM Application where cor_id = $cor_id";
    ...
    while ($row = $result->fetch_assoc()) {
       ...
       // $output .= "<td>" . htmlspecialchars($row["notes"]) . "</td>";
        ...
        . $row["decision2"] . "\", \"". $row["coordinator"] . "\", \"". $row["notes"] . "\")'><i class='fas fa-edit'></i></button> ";
   ...
   // echo "</pre>";
    ...
    if ($action == 'Add') {
       ...
       // $student_id = $conn->real_escape_string($_POST['student_id']);
        ...
        $decision2 = $conn->real_escape_string($_POST['decision2']);
        $coordinator = $conn->real_escape_string($_POST['coordinator']);
        $decisiondate2 = date('Y-m-d');
        $sql = "UPDATE application SET decision2 = '$decision2', coordinator = '$coordinator', decision2date = '$decisiondate2' WHERE id = $id";
