...
function displayData($conn) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $output .= "<button class='btn' onclick='editBtn(\"" . $row["id"] . "\", \"" . htmlspecialchars($row["submiton"], ENT_QUOTES) . "\", \"" . $row["decision"] . "\", \"" . $row["decisiondate"] . "\", \"". $row["student"] . "\", \"". htmlspecialchars($row["notes"], ENT_QUOTES) . "\")'><i class='fas fa-edit'></i></button> ";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == 'Add') {
        ...
        $sql = "INSERT INTO insuranceform (submiton, decision, notes, student_id) VALUES ('$title', '$uploadon', '$notes', '$student_id')";
    } elseif ($action == 'Edit') {
